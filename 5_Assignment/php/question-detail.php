    ...
    <main class="page-main" id="mp-after-login-body">
        ...
        <article class="page-main-body">
            ...
            if(!$match['numAnswers']){
                ...
                $query = "SELECT a.user_id, a.title, a.answer, a.date_posted, a.answer_id, u.username, u.profile_photo, SUM(v.vote_status) AS votes
                FROM answers AS a
                INNER JOIN users AS u
            ...
            ?>
                        <sections class="card answer-card" id="<?=$aid?>">

                            <div class="card-vote-active">
                            <form action="includes/avote.inc.php?qid=<?=$qid?>&aid=<?=$match['answer_id']?>&auid=<?=$match['user_id']?>&vs=1" class="vote-btn 
                            <?php
                            if(!empty($match2)){
                                if($match2['vote_status'] == 1 && $match2['user_id'] == $uid){
                                    echo "vote-active-green";
                                }
                                else {
                                    echo "";   
                                }
                            ...
                            ?>" method="post">
                                <input type="image" name="1" class="vote-btn" src="../images/vote/30-arrow-up.png" alt="UP">
                            </form>

                            <p class="comment-vote-num"><?=$match['votes']?></p>

                            <form action="includes/avote.inc.php?qid=<?=$qid?>&aid=<?=$match['answer_id']?>&auid=<?=$match['user_id']?>&vs=-1" class="vote-btn 
                            <?php
                            if(!empty($match2)){
                                if($match2['vote_status'] == -1 && $match2['user_id'] == $uid){
                                    echo "vote-active-red";
                                }
                                else {
                                    echo "";   
                                }
                            ...
                            ?>" method="post">
                                <input type="image" name="-1" class="vote-btn" src="../images/vote/30-arrow-down.png" alt="DOWN">
                            </form>
